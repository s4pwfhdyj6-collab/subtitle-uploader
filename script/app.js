...
class SubtitleUploader {
    ...
    initializeElements() {
        ...
        this.uploadButtonContainer = document.getElementById('uploadButtonContainer');
    ...
    handleFiles(fileList) {
        ...
        if (newFiles.length === 0) {
            this.showAlert('지원하지 않는 파일 형식입니다. SRT, VTT, ASS, SSA, TXT 파일만 업로드 가능합니다.', 'danger');
            return;
        }
    ...
    async uploadFiles() {
        ...
        this.uploadBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>업로드 중...';
        ...
        try {
            ...
            // 성공 메시지
            this.showAlert(`성공적으로 ${this.files.length}개의 파일을 업로드했습니다!`, 'success');
            ...
            this.showAlert(`업로드 중 오류가 발생했습니다: ${error.message}`, 'danger');
        } finally {
            // 업로드 버튼 활성화
    ...
    async simulateUpload(formData) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const interval = setInterval(() => {
                ...
                this.progressText.textContent = `${this.files.length}개 파일 중 ${Math.floor(progress / 100 * this.files.length)}개 업로드 완료`;
            }, 200);
        });
